<script setup>
// 删除会话二次确认弹窗（危险操作风格，替代原生 confirm）
defineProps({
  // 会话名称（确认文案展示）
...
<template>
  <div class="confirm-overlay" @click.self="emit('cancel')">
    <div class="confirm-modal" role="dialog" aria-modal="true" aria-label="删除会话确认">
      ...
      <p class="confirm-text">确定删除「{{ convName }}」吗？</p>
      <p class="confirm-sub">删除后本地聊天记录将一并清除，服务端消息仍保留；再次收发消息会自动重建会话。</p>
      <div class="confirm-actions">
        <button class="confirm-btn cancel" @click="emit('cancel')">取消</button>
        <button class="confirm-btn danger" :disabled="deleting" @click="emit('confirm')">
          {{ deleting ? '删除中…' : '删除' }}
        </button>
      </div>
    </div>
